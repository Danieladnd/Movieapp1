...
const API_URL = 'https://danieladnd2.pythonanywhere.com/'; /* 
  'http://localhost:8000'
...
export class APIService {
  ...
  addNewMovie(movie) {
    // Ensure 'director' field is included in the movie object
    const url = `${API_URL}/api/movies/`;
    let jwtToken = localStorage.getItem('access');
    const headers = { Authorization: `JWT ${jwtToken}` };
  ...
  updateMovie(movie) {
    // Ensure 'director' field is included when updating a movie
